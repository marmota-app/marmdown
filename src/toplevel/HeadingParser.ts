...
import { ContainerTextParser, defaultSkipLineStart, ParserResult, TextParser } from "$markdown/parser/TextParser";
...
export class HeadingParser extends ContainerTextParser<UpdatableHeading, string | Options> implements TextParser<UpdatableHeading> {
	...
	parse(text: string, start: number, length: number, skipLineStart = defaultSkipLineStart): ParserResult<UpdatableHeading> | null {
		let i = 0

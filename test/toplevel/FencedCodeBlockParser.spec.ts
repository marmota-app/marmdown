...
describe('FencedCodeBlockParser', () => {
	...
	it('parses text content until end of document when there is no closing fence', () => {
		const md = '```\ncontent\n'
	...
	it('parses options after starting fence', () => {
		const md = '```{ foo }\ncontent\n'
	...
	fenceTestData.forEach(td => it(`parses code block from ${td[0]} to ${td[1]} as ${JSON.stringify(td[3])}`, () => {
	...
	lengthTestData.forEach(([md, start, expectedLength])=>it(`produces result of length ${expectedLength} for markdown "${md}"`, () => {
